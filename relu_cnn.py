...
for i in range(MAX_ITER):
    ...
    if i % 20 == 0:
        x_data.append(i)
        # 计算训练准确率
        step_train_acc = accuracy.eval(feed_dict={x_holder: batch[0], y_holder: batch[1], drop_prob: 1.0})
        tra_train_acc.append(step_train_acc)

        # 计算测试准确率
        step_test_acc = accuracy.eval(feed_dict={x_holder: test_batch[0], y_holder: test_batch[1], drop_prob: 1.0})
        tra_test_acc.append(step_test_acc)

        # 计算代价函数
        temp_j = cross_entropy.eval(feed_dict={x_holder: batch[0], y_holder: batch[1], drop_prob: 1.0})
        tra_cross.append(temp_j)
        # # print("代价：", temp_j)

    if i % 50 == 0:
        print("at step %s ,accuracy is %s" % (i, step_train_acc))
